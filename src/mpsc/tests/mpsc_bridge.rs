...
use crate::mpsc::errors;
use crate::mpsc::RecvRef;
...
fn mpsc_try_send_recv() {
    loom::model(|| {
        ...
        while let Some(val) = rx.blocking_recv_ref() {
            vals.push(*val);
        }
...
fn mpsc_try_recv_ref() {
    loom::model(|| {
        ...
        let p1 = {
            let tx = tx.clone();
            thread::spawn(move || {
                *tx.blocking_send_ref().unwrap() = 1;
                thread::yield_now();
                *tx.blocking_send_ref().unwrap() = 2;
            })
        };
        let p2 = thread::spawn(move || {
            *tx.blocking_send_ref().unwrap() = 3;
            thread::yield_now();
            *tx.blocking_send_ref().unwrap() = 4;
        });
...
fn rx_close_unconsumed_spsc() {
    ...
    loom::model(|| {
        ...
        let mut i = 1;
        while let Ok(mut slot) = tx.blocking_send_ref() {
...
fn rx_close_unconsumed_mpsc() {
    ...
    fn do_producer(tx: Sender<Track<i32>>, n: usize) -> impl FnOnce() + Send + Sync {
        move || {
            let mut i = 1;
            while let Ok(mut slot) = tx.blocking_send_ref() {
...
fn spsc_recv_then_try_send() {
    loom::model(|| {
        ...
        let consumer = thread::spawn(move || {
            assert_eq_dbg!(rx.blocking_recv().unwrap(), 10);
        });
...
fn spsc_recv_then_close() {
    loom::model(|| {
        ...
        assert_eq_dbg!(rx.blocking_recv(), None);
...
fn spsc_recv_then_try_send_then_close() {
    loom::model(|| {
        ...
        let consumer = thread::spawn(move || {
            assert_eq_dbg!(rx.blocking_recv().unwrap(), 10);
            assert_eq_dbg!(rx.blocking_recv().unwrap(), 20);
            assert_eq_dbg!(rx.blocking_recv(), None);
        });
...
fn spsc_send_recv_in_order_no_wrap() {
    ...
    loom::model(|| {
        ...
        let consumer = thread::spawn(move || {
            for i in 1..=N_SENDS {
                assert_eq_dbg!(rx.blocking_recv(), Some(i));
            }
            assert_eq_dbg!(rx.blocking_recv(), None);
        });
...
fn spsc_send_recv_in_order_skip_wrap() {
    ...
    loom::model(|| {
        ...
        let consumer = thread::spawn(move || {
            ...
            assert_eq_dbg!(rx.blocking_recv(), Some(1));
            loop {
                match rx.try_recv_ref() {
                    Ok(val) => {
                        assert_eq_dbg!(*val, 2);
